...
for _ in $(seq 1 60); do
  if curl -fsS "http://127.0.0.1/widget/v1/health" >/dev/null 2>&1; then OK=1; break; fi
  sleep 5
done
if [ -z "$OK" ]; then
...
VERSION_REPORTED="$(curl -fsS http://127.0.0.1/widget/v1/health | sed -E 's/.*"version":"([^"]+)".*/\1/')"
