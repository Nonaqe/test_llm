...
Handoff при отсутствии операторов онлайн: AI сообщает, что операторов нет, и предлагает оставить email. Заявка (`leave-email`) фиксируется в диалоге и `events`; диалог → `RESOLVED` (заявка обработана людьми). Возврат операторов не «воскрешает» диалог автоматически.

## 5. Уведомления и presence

- Операторы: online / away / offline (heartbeat presence).
- Handoff: WS-тост + звук в панели; email через SMTP заказчика, если никто не принял за N минут (настройка).
- Посетителю: статус «оператор онлайн/офлайн», typing-индикатор оператора.
