...
`tsv` генерируется конфигурацией `'simple'` — и в примере, и в реализации MVP (`apps/api/migrations/0002_domain_v1.sql`); ru/en stemming-конфигурация в MVP НЕ применяется — выбор стемминга отложен до V2 либо падения FTS-полноты на eval-наборе (см. D-2 в DOC-031 §4). Все запросы фильтруются по `project_id` — изоляция арендаторов на уровне данных.
